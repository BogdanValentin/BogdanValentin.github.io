...
class PreloaderManager {
  ...
  createLoadingScreen() {
    ...
    // The gate is ~2.8s long either way, so it may as well say whose site this is
...
class FashionGallery {
  ...
  createScalingOverlay(sourceImg) {
    ...
    // Positioning is handled entirely by enterZoomMode, so do not set it here
  ...
  enterZoomMode(selectedItemData) {
    ...
    // Contained image rect within grid cell: the actual rendered content area to animate FROM.
  ...
  _scheduleMasonryRecompute() {
    ...
    this._masonryRecomputeTimer = setTimeout(() => {
      ...
      // would kill the active drag, so re-defer until the drag ends
  ...
  buildCategoryIndex() {
    ...
    allRow.innerHTML = `
      ...
      <span class="category-count">${totalPhotos > 0 ? totalPhotos : '0'}</span>
    `;
    ...
    GALLERY_CATEGORIES.forEach((cat, i) => {
      ...
      row.innerHTML = `
        ...
        <span class="category-count">${cat.images.length > 0 ? cat.images.length : '0'}</span>
      `;
  ...
  /* ---- Navigation: the URL is the single source of truth ----------------
     ...
     index, because initImageData and switchCategory both shuffle, so index 47 is
  ...
  initGestureZoomBlock() {
    ...
    // pinch, so cancelling its gesture events is the only lever, and document
